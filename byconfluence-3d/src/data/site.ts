...
export type WorkItem = {
  ...
  seed: number
  /**
   * Optional real photograph for this frame, as a site-relative path
   * ('photos/work-01.jpg' — no leading slash; the loader prefixes Vite's
   * BASE_URL so it works on sub-path deploys). When the file exists it
   * crossfades in over the procedural plate; when it is missing or fails to
   * load, the procedural plate simply stays. Populate `public/photos/` via
   * `tools/fetch-photos.mjs` (run on a machine with open network access) or by
   * dropping files in.
   */
  photo?: string
}
